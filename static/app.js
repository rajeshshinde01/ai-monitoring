...
let trendSeries = { cpu: true, memory: true, errors: true, warnings: true, restarts: true };
let investigationPodName = '';
let selectedDeploymentName;
let selectedDeploymentResource;
let selectedConnectedServiceName = '';
let podSearch = '';
let podFilter = 'all';
...
function renderCorrelationIntelligence() {
  ...
  target.querySelectorAll('[data-intelligence-focus]').forEach(button => button.addEventListener('click', () => {
    if (button.dataset.intelligenceFocus === 'alerts') {
      window.history.replaceState(null, '', '#alerts'); setWorkspacePage('alerts'); window.scrollTo({ top: 0, behavior: 'smooth' }); return;
    }
    target.querySelector('.correlation-list')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }));
}
...
function renderUrlMonitors() {
  ...
  const draft = editing || urlMonitorPrefill || {};
  const isPrefill = !editing && Boolean(urlMonitorPrefill);
  admin.innerHTML = isAdministrator() ? `<details class="url-monitor-add" ${(editing || isPrefill) ? 'open' : ''}><summary><span>${editing ? '✎' : '＋'}</span><div><strong>${editing ? 'Edit application endpoint' : isPrefill ? 'Review discovered endpoint' : 'Add an application endpoint'}</strong><small>${editing ? 'Update the URL, monitored path, expected response, or monitoring state' : isPrefill ? 'Confirm the discovered Route or Ingress before enabling continuous monitoring' : 'Configure an approved URL for continuous environment monitoring'}</small></div><b>${editing ? 'Editing' : isPrefill ? 'Review' : 'Open form'}</b></summary><form id="url-monitor-form"><label>Application name<input name="name" required maxlength="120" placeholder="Customer portal" value="${escapeHtml(draft.name || '')}"></label><label>Environment<select name="environment">${environments.map(environment => `<option value="${environment}" ${draft.environment === environment ? 'selected' : ''}>${environment.toUpperCase()}</option>`).join('')}</select></label><label class="wide-field">Base application URL<input name="url" type="url" required placeholder="https://app-dev.example.com" value="${escapeHtml(draft.url || '')}"></label><label>Monitored path<input name="health_path" placeholder="/navigator-frame or /health" value="${escapeHtml(draft.health_path || '')}"></label><label>Expected response<input name="expected_status" type="number" min="100" max="599" value="${Number(draft.expected_status || 200)}"></label><label class="url-monitor-enabled"><input name="enabled" type="checkbox" ${draft.enabled === false ? '' : 'checked'}><span>Enable continuous checks</span></label><button type="submit" class="primary">${editing ? 'Save changes' : 'Add endpoint'}</button>${(editing || isPrefill) ? '<button type="button" class="url-edit-cancel" data-url-edit-cancel>Cancel</button>' : ''}<p class="form-message"></p></form></details>` : '<p class="rule-view-only">Administrators manage URLs. You can review and refresh their current status.</p>';
  admin.querySelector('#url-monitor-form')?.addEventListener('submit', saveUrlMonitor);
  admin.querySelector('[data-url-edit-cancel]')?.addEventListener('click', () => { urlMonitorEditingId = ''; urlMonitorPrefill = null; renderUrlMonitors(); });
}
...
async function saveUrlMonitor(event) {
  ...
  urlMonitorEnvironment = payload.environment; urlMonitorEditingId = ''; urlMonitorPrefill = null; urlMonitorActionMenu = ''; form.reset(); await loadUrlMonitors();
}
...
function svgSeries(samples, selector, color, width = 760, height = 188, maximum = null, label = 'Value', suffix = '%') {
  ...
  const markers = points.map((point, index) => { const [x, y] = point.split(','); const timestamp = new Date(samples[index].timestamp * 1000).toLocaleTimeString(); const reading = `${timestamp} · ${label}: ${values[index]}${suffix}`; return `<circle class="trend-hit" cx="${x}" cy="${y}" r="7" tabindex="0" role="button" data-trend-reading="${escapeHtml(reading)}"><title>${escapeHtml(reading)}</title></circle>`; }).join('');
...
function renderObservability(report) {
  ...
  const legendButton = (key, label) => `<button type="button" class="${key} ${trendSeries[key] ? 'active' : ''}" data-trend-series="${key}" aria-pressed="${trendSeries[key]}">${label}</button>`;
  const trendLegend = capacityView ? `<div class="trend-legend">${legendButton('cpu', 'CPU usage')}${legendButton('memory', 'Memory usage')}<small>Toggle a series · peak ${capacityMaximum}%</small></div>` : `<div class="trend-legend">${legendButton('errors', 'Errors')}${legendButton('warnings', 'Warnings')}${legendButton('restarts', 'Restarts')}<small>Toggle a series · peak ${eventMaximum}</small></div>`;
  const chartWidth = 920;
  const chartHeight = 250;
  const trendLines = capacityView
    ? `${trendSeries.cpu ? svgSeries(samples, item => item.summary.cpu, '#416ce4', chartWidth, chartHeight, chartMaximum, 'CPU', '%') : ''}${trendSeries.memory ? svgSeries(samples, item => item.summary.memory, '#845bd4', chartWidth, chartHeight, chartMaximum, 'Memory', '%') : ''}`
    : `${trendSeries.errors ? svgSeries(samples, item => item.summary.errors, '#d26751', chartWidth, chartHeight, eventMaximum, 'Errors', ' events') : ''}${trendSeries.warnings ? svgSeries(samples, item => item.summary.warnings, '#d8912b', chartWidth, chartHeight, eventMaximum, 'Warnings', ' events') : ''}${trendSeries.restarts ? svgSeries(samples, item => item.summary.restarts, '#7565c8', chartWidth, chartHeight, eventMaximum, 'Restarts', ' total') : ''}`;
  ...
  document.querySelectorAll('[data-trend-series]').forEach(button => button.addEventListener('click', () => { trendSeries[button.dataset.trendSeries] = !trendSeries[button.dataset.trendSeries]; renderObservability(observabilityData); }));
  const showTrendReading = point => { const reading = document.querySelector('.trend-reading'); if (reading) reading.textContent = point.dataset.trendReading; };
  ...
  const map = report?.service_map || { nodes: [], edges: [], connections: [], local_services: [] };
  const connections = map.connections || [];
  const localServices = map.local_services || [];
  const monitorButton = connection => isAdministrator() && connection.endpoint ? `<button type="button" class="topology-monitor" data-topology-monitor-url="${escapeHtml(encodeURIComponent(connection.endpoint))}" data-topology-monitor-name="${escapeHtml(encodeURIComponent(connection.service))}">Add to URL Monitoring</button>` : '';
  document.querySelector('#service-map').innerHTML = map.mode === 'docker' ? (localServices.length ? `<div class="docker-topology-intro"><strong>Docker runtime inventory</strong><span>Local Docker does not expose Kubernetes Routes, Ingresses, or Service selectors. Select a service to inspect its workload.</span></div><div class="local-service-grid">${localServices.map(service => `<button type="button" class="local-service-card" data-service-workload="${escapeHtml(service.name)}"><div class="local-service-name"><span>Service</span><strong>${escapeHtml(service.name)}</strong></div><div class="local-service-endpoint"><span>Network access</span><small>${escapeHtml((service.ports || []).join(' · ') || 'Internal network only')}</small></div><b class="${severityClass(service.status || 'healthy')}">${escapeHtml(service.status || 'healthy')}</b></button>`).join('')}</div>` : '<p class="empty">No local Docker services are available.</p>') : (connections.length ? `<p class="service-map-note">Verified Route/Ingress and Kubernetes Service selector links only. Application call paths require tracing.</p><div class="service-connection-list">${connections.map(connection => `<article class="service-connection topology-connection">${connection.endpoint ? `<div class="topology-endpoint"><span>${escapeHtml(connection.endpoint_kind)}</span><a href="${escapeHtml(connection.endpoint)}" target="_blank" rel="noopener noreferrer">${escapeHtml(connection.endpoint)} ↗</a>${monitorButton(connection)}</div><i>→<small>routes to</small></i>` : `<div class="topology-endpoint internal"><span>Internal service</span><strong>No external route</strong></div><i>→</i>`}<div class="topology-service"><span>Kubernetes Service</span><strong>${escapeHtml(connection.service)}</strong><small>${escapeHtml(connection.ports)}</small></div><i>→<small>selects</small></i><button type="button" data-service-workload="${escapeHtml(connection.workload)}"><span class="${severityClass(connection.status || 'healthy')}">${escapeHtml(connection.status || 'healthy')}</span><strong>${escapeHtml(connection.workload)}</strong><small>${escapeHtml(connection.workload_kind)}</small></button></article>`).join('')}</div>` : '<p class="empty">No verified Route, Ingress, Service, or workload links were found. This map does not infer application call paths.</p>');
  ...
  document.querySelectorAll('[data-topology-monitor-url]').forEach(button => button.addEventListener('click', () => {
    urlMonitorEditingId = '';
    urlMonitorPrefill = { name: decodeURIComponent(button.dataset.topologyMonitorName), url: decodeURIComponent(button.dataset.topologyMonitorUrl), environment: urlMonitorEnvironment, health_path: '', expected_status: 200, enabled: true };
    window.history.pushState(null, '', '#url-monitoring');
    setWorkspacePage('url-monitoring');
    renderUrlMonitors();
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }));
  const dependencies = report?.dependencies || [];
  if (!dependencies.some(item => item.name === selectedConnectedServiceName)) selectedConnectedServiceName = dependencies.find(item => item.status !== 'healthy')?.name || dependencies[0]?.name || '';
  const selectedService = dependencies.find(item => item.name === selectedConnectedServiceName);
  const selectedPod = data?.pods?.find(item => item.name === selectedService?.pod);
  const latestSignal = (report?.events || []).find(event => event.pod === selectedService?.pod || event.workload === selectedService?.name);
  document.querySelector('#dependencies').innerHTML = dependencies.length ? dependencies.map(item => `<button class="dependency-row ${item.name === selectedConnectedServiceName ? 'active' : ''}" data-connected-service="${escapeHtml(item.name)}"><div><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.type)} · ${escapeHtml(item.detail)}</small></div><span class="${severityClass(item.status)}">${escapeHtml(item.status)}</span></button>`).join('') : '<p class="empty">No connected workloads are available in the current service map.</p>';
  document.querySelector('#dependency-summary').innerHTML = selectedService ? `<p class="eyebrow">SELECTED SERVICE</p><h3>${escapeHtml(selectedService.name)}</h3><span class="${severityClass(selectedService.status)}">${escapeHtml(selectedService.status)}</span><div><span>Runtime type</span><strong>${escapeHtml(selectedService.type)}</strong></div><div><span>Pod state</span><strong>${escapeHtml(selectedPod?.status || 'Not reported')}</strong></div><div><span>Latest signal</span><strong>${escapeHtml(latestSignal?.title || 'No recent signal')}</strong></div><p>${escapeHtml(latestSignal?.detail || 'No recent event was recorded for this workload.')}</p><button type="button" class="dependency-open-logs" data-connected-service-logs="${escapeHtml(selectedService.name)}">Open pod logs</button>` : '<p class="empty">Select a connected service to review its runtime status and latest evidence.</p>';
  document.querySelectorAll('[data-connected-service]').forEach(button => button.addEventListener('click', () => { selectedConnectedServiceName = button.dataset.connectedService; renderObservability(report); }));
  document.querySelector('[data-connected-service-logs]')?.addEventListener('click', () => { if (selectedService.pod) openPodInvestigation(selectedService.pod); });
  const events = report?.events || [];
...
function selectPod(name, resetTab = true) {
  ...
  const forecastHeadline = forecast.available ? `${forecast.forecast_4h_percent ?? forecast.forecast_percent}%` : `${forecast.current_percent}%`;
  const forecastLabel = forecast.available ? 'four-hour projected memory usage' : 'live memory usage';
  const forecastDetail = forecast.available ? `1h ${forecast.forecast_1h_percent ?? '—'}% · 4h ${forecast.forecast_4h_percent ?? forecast.forecast_percent}% · ${forecast.coverage_hours ?? 0}h retained history` : `${forecast.current_memory_mib} MiB of ${forecast.limit_mib} MiB`;
  ...
  const forecastOverview = `<section class="forecast-overview"><header><span>ALL MONITORED PODS</span><small>${allForecasts.length} compared · highest four-hour projected use first</small></header><div>${allForecasts.map(item => { const value = Number(item.available ? (item.forecast_4h_percent ?? item.forecast_percent) : item.current_percent); const risk = item.available ? item.forecast_risk : item.risk; return `<button type="button" class="${item.pod === pod.name ? 'active' : ''}" data-forecast-pod="${escapeHtml(item.pod)}"><div><b>${escapeHtml(item.pod)}</b><small>${item.available ? `4-hour projection · ${item.coverage_hours ?? 0}h history` : 'Current memory use'}</small></div><strong>${value}%</strong><span class="${severityClass(risk)}">${escapeHtml(risk)}</span></button>`; }).join('')}</div></section>`;
