...
function renderObservability(report) {
  ...
  const selectedPod = data?.pods?.find(item => item.name === selectedService?.name);
  const latestSignal = (report?.events || []).find(event => event.pod === selectedService?.name || event.workload === selectedService?.name);
  ...
  document.querySelector('[data-connected-service-logs]')?.addEventListener('click', () => openPodInvestigation(selectedService.name));
  const events = report?.events || [];
