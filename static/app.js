...
function renderObservability(report) {
  ...
  document.querySelector('#service-map').innerHTML = map.edges.length ? `<p class="service-map-note">Verified Kubernetes Service selectors only. Application call paths require tracing.</p><div class="service-connection-list">${map.edges.map(edge => { const source = nodeById.get(edge.from); const target = nodeById.get(edge.to); return `<article class="service-connection"><div><span>Service</span><strong>${escapeHtml(source?.label || 'Service')}</strong></div><i>→<small>${escapeHtml(edge.label)}</small></i><button type="button" data-service-workload="${escapeHtml(target?.label || '')}"><span class="${severityClass(target?.status || 'healthy')}">${escapeHtml(target?.status || 'healthy')}</span><strong>${escapeHtml(target?.label || 'Workload')}</strong><small>${escapeHtml(target?.kind || 'Workload')}</small></button></article>`; }).join('')}</div>` : '<p class="empty">No verified Service-to-workload connections were found. This map does not infer application call paths.</p>';
